...
func signup(c *fiber.Ctx) error{
    ...
    if db.First(&client, "username = ?", client.Username).RecordNotFound() {
    // User does not exist, proceed with creating a new one
} else {
    return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"User Already Exists"})
}
...
func login(c *fiber.Ctx) error {
    ...
    expectedPassword := db.First(&users,creds.Username).Select("Password")
