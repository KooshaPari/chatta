...
func login(c *fiber.Ctx) error {
    ...
    db.First(&users, db.Where("username = ?", creds.Username).First(&users).Select("Password")).Select("Password")
    expectedPassword := users.Password
    if bcrypt.CompareHashAndPassword([]byte(expectedPassword),[]byte(creds.Password))!=nil {
