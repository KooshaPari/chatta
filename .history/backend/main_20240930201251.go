...
func main() {
initDB()
Hashed password from DB: illegal base64 data at input byte 0
Hashed password from DB: crypto/bcrypt: hashedSecret too short to be a bcrypted password
app := fiber.New()
...
func login(c *fiber.Ctx) error {
   ...
   db.Where("username = ?", creds.Username).First(&users).Select("Password")
    expectedPassword, err :=  base64.StdEncoding.DecodeString(users.Password)
