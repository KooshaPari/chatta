...
app.Use(cors.New(cors.Config{
    ...
    app.Get("/ws", websocket.New(func(c *websocket.Conn) {
		...
		for {
            ...
            incomingMsg.SentAt = time.Now()             // Set current timestamp
            incomingMsg.Edited = false                   // Default to not edited
            incomingMsg.SenderID = users.UUID              // Assuming you have the sender's UUID
            incomingMsg.Sender = user s             // Assuming you have the current user object
            ...
            fullMsg := fmt.Sprintf("%s: %s", username, string(incomingMsg.Content))
