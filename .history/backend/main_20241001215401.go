...
app.Use(cors.New(cors.Config{
    ...
    app.Get("/ws", websocket.New(func(c *websocket.Conn) {
		...
		for {
            ...
            if incomingMsg.Edited {
                fmt.Println("Editing Message: ", incomingMsg.Content)
                 oldmsg:= new(models.Message);
                 db.Where("uuid = ?", incomingMsg.UUID).First(&oldmsg)
               oldmsg.Content = incomingMsg.Content
               db.Where("uuid = ?", incomingMsg.UUID).Update("content",incomingMsg.Content)
