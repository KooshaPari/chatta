			...
			if err != nil{
                ...
                if incomingMsg.Deleted{
                    ...
                    if (incomingMsg.Channel != "0000"){
                        print("Adding to Appropriate Array.")
                        // add to the chat with UUID same as channels' messages array
                        chat := new(models.Chat)
                        db.Where("uuid = ?", incomingMsg.Channel).First(chat)
...
func getChat(c *fiber.Ctx) error{
    ...
    if err := db.Model(&models.Chat{}).Where("uuid = ?", uuid).First(&chat).Error; err != nil {
...
func getMessages(c *fiber.Ctx) error {
    ...
    if err := db.Preload("Sender").b.Find(&messages).Error; err != nil {
    return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": err.Error()})
}
