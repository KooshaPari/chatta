...
func getDM(c *fiber.Ctx) error{
    ...
    if err := db.Where("type = ? AND participant_uuids @> ARRAY[?, ?]", "dm", tempChat.SenderUUIDs[0], tempChat.SenderUUIDs[1]).First(&tempChat).Error; err != nil {
        if err == gorm.ErrRecordNotFound {
            tempChat.UUID = uuid.New()
        }else{
        // Log the error for debugging purposes
        fmt.Printf("Database error: %v\n", err)
        return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
    ...
    // If Y: Return Chat OBJ

    // IF N: Create a New One, add to DB, Return
