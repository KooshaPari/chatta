...
func getDM(c *fiber.Ctx) error{
    ...
    if err := db.Joins("JOIN dm_participants ON dm_participants.chat_id = chats.uuid").
    Where("chats.type = ? AND dm_participants.user_id IN (?, ?)", "dm", participant1UUID, participant2UUID).
    Group("chats.uuid").
    Having("COUNT(DISTINCT dm_participants.user_id) = 2").First(&existingChat).Error; err == nil {
