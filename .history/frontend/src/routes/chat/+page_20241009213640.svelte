<script lang="ts">
	...
	function directMessage(sender: User){
		// check for an existing chatDM first, (figure out how to actually do this)
		// if it doesnt exist make a new one
			// create a new chat object with appropriate type, send to backend and ensure it is associated excl with both users
		// else just switch to that
		switchChannel(newDM.uuid);
	}
...
<div class="chattaApp">
	<div class="chattaSidebar">
		<menu class="chattaNav">
			...
			{#each chats as chat}
				{#if chat.type == "DirectMessage"}<button
						on:click={() => switchChannel(chat)}
						class="chatPill">{chat.name}</button
					>{/if}
			{/each}
			<li>
				DMs
					...
					{#each messages as msg, index}
						{#if msg.channel == wsChannel}
							...
							>
								<div class="textCont">
									<button
										on:click={() => directMessage(msg.sender)}
