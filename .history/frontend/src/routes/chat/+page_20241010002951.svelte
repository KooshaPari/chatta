<script lang="ts">
	...
	function handleSignaling(data){
		// There are offer answer and candidate signals, offer->answer(acc/deny)->candidate
	...
	function Call(caller: User, receiver: User) {
		...
		.then(stream => {
			localStream = stream;
			document.querySelector('chat-window').srcObject = localStream;
	...
	<div class="chattaRight">
		...
		<div class="dm-window">
			...
			<div class="chat-window">
				<VideoModal isOpen={onCall} on:close={(onCall = !onCall)}
					><h1>Video Call</h1>
					<video id="localVideo" autoplay playsinline></video>
					<video id="remoteVideo" autoplay playsinline></video>

					<button on:click={() => startCall(client, receiver)}
						>Start Call</button
					>
				</VideoModal>
