...
func getDM(c *fiber.Ctx) error{
    ...
    if err := db.Preload("Messages").Preload("Messages.Sender").Preload("Participants").Joins("JOIN dm_participants ON dm_participants.chat_uuid = chats.uuid").
...
func getChats(c *fiber.Ctx) error{
    chats := []models.Chat{}
    if err := db.Preload("Participants").Find(&chats).Error; err != nil {
